...
int main(int argc, char **argv) {
  ...
  const std::string configFile =
      kDataDir + "/depth_anything/config_hypersim.yaml";
#else
  ...
  auto processImage = [&](const cv::Mat frame, cv::Mat &result_d) {
    // cout << "Predicting for input image: " << frame.size() << ", " <<
    // cvTypeToStr(frame.type()) << endl;

    auto start = std::chrono::system_clock::now();
    result_d = depth_model.predict(frame);
    auto end = chrono::system_clock::now();
    cout << "Time per output frame (" << result_d.size << ", "
         << cvTypeToStr(result_d.type()) << "): "
         << chrono::duration_cast<chrono::milliseconds>(end - start).count()
         << "ms" << endl;

    // Resize predicted depth image to the same size as the input image with
    // nearest neighbor interpolation
    cv::resize(result_d, result_d, cv::Size(frame.cols, frame.rows), 0, 0,
               cv::INTER_NEAREST);
    // cout << "Scaled output image: " << result_d.size() << ", " <<
    // cvTypeToStr(result_d.type()) << endl;
    ...
    getPointCloudFromImageAndDepth<PointT, float>(
        frame, result_d, emptyMask, intrinsics, border, Twc, pointcloud);
  ...
  if (isVideo) {
    ...
    // cv::VideoWriter output_video("output_video.avi",
    // cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 30, cv::Size(width,
    // height));
    while (1) {
      cv::Mat frame;
      cap >> frame;
