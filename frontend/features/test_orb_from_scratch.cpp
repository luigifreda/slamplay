...
inline int DescriptorDistance32(const vector<uint32_t> &a, const vector<uint32_t> &b);
inline int DescriptorDistanceSIMD32(const vector<uint32_t> &a, const vector<uint32_t> &b);
inline int DescriptorDistanceSIMD64(const vector<uint32_t> &a, const vector<uint32_t> &b);

int main(int argc, char **argv) {
    // load image
    cv::Mat first_image = cv::imread(img1_file, 0);
    cv::Mat second_image = cv::imread(img2_file, 0);
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
    ...
    for (auto &kp : keypoints) {
        ...
        DescType desc(8, 0);  // vector of 8 elements, each one a uint32_t, 8x32=256
        for (int i = 0; i < 8; i++) {
            uint32_t d = 0;
            for (int k = 0; k < 32; k++) {
                int idx_pq = i * 32 + k;
...
#define CHECK 0
#if CHECK
            distance = DescriptorDistance32(desc1[i1], desc2[i2]);
            int distanceSIMD32 = DescriptorDistanceSIMD32(desc1[i1], desc2[i2]);
            int distanceSIMD64 = DescriptorDistanceSIMD64(desc1[i1], desc2[i2]);
            MSG_ASSERT(distance == distanceSIMD32, "distance must be equal to distanceSIMD32");
            MSG_ASSERT(distanceSIMD32 == distanceSIMD64, "distanceSIMD32 must be equal to distanceSIMD64");
#else
            // distance = DescriptorDistance32(desc1[i1], desc2[i2]);
            // distance = DescriptorDistanceSIMD32(desc1[i1], desc2[i2]);
            distance = DescriptorDistanceSIMD64(desc1[i1], desc2[i2]);
#endif
...
int DescriptorDistance32(const vector<uint32_t> &a, const vector<uint32_t> &b) {
    const uint32_t *pa = &a[0];
    const uint32_t *pb = &b[0];
...
// Bit set count operation from
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
int DescriptorDistanceSIMD32(const vector<uint32_t> &a, const vector<uint32_t> &b) {
    const uint32_t *pa = &a[0];
    const uint32_t *pb = &b[0];
...
// Bit set count operation from
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
int DescriptorDistanceSIMD64(const vector<uint32_t> &a, const vector<uint32_t> &b) {
    const uint64_t *pa = (uint64_t *)&a[0];
    const uint64_t *pb = (uint64_t *)&b[0];
