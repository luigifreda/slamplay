#include "ros/pointcloud_convert/packets_parser.h"
...
inline double TimeToMicros(const ros::Time &stamp) {
  return static_cast<double>(stamp.sec) * kMicrosPerSecond +
         static_cast<double>(stamp.nsec) / 1e3;
}

inline float ComputeAzimuthCorrection(int firing, int dsr, float azimuth_diff) {
  const float point_time_offset = dsr * PacketsParser::kVLP16DsrToffset +
                                  firing * PacketsParser::kVLP16FiringToffset;
  return azimuth_diff * point_time_offset / PacketsParser::kVLP16BlockTduration;
}
...
int PacketsParser::Setup(const VelodyneConfig &conf) {
  ...
  for (uint16_t rot_index = 0; rot_index < kRotationMaxUnits; ++rot_index) {
    const float rotation = kRotationResolution * rot_index * kDegreesToRadians;
  ...
  for (auto iter = ordered_vert_angle.begin(); iter != ordered_vert_angle.end();
       ++iter, index++) {
...
void PacketsParser::PaddingPointCloud(const PacketsMsgPtr &scan_msg,
  ...
  if (!out_pc_msg_ptr) {
    out_pc_msg_ptr.reset(new FullPointCloudType());
  }

  out_pc_msg_ptr->clear();
  ...
  for (size_t i = 0; i < packets_size; ++i) {
    Unpack(scan_msg->packets[i], scan_msg->header.stamp,
           organized_raw_pointcloud_);
  }

  ArrangePointcloud(organized_raw_pointcloud_, out_pc_msg_ptr);

  out_pc_msg_ptr->header = sad::ToPclHeader(scan_msg->header);

  if (out_pc_msg_ptr->empty()) {
    //        LOG(ERROR) << "All points is NAN!Please check velodyne:" <<
    //        config_.model;
  }
}
...
void PacketsParser::Unpack(const velodyne_msgs::VelodynePacket &pkt,
  ...
  const double pkt_time_us = TimeToMicros(pkt.stamp);
  const double msg_time_us = TimeToMicros(msg_stamp);
  ...
  for (int block = 0; block < kBlocksPerPacket; block++) {
    ...
    for (int firing = 0, k = 0; firing < kVLP16FiringsPerBlock; ++firing) {
      for (int dsr = 0; dsr < kVLP16ScanPerFiring; ++dsr, k += kRawScanSize) {
        ...
        azimuth_corrected_f =
            azimuth + ComputeAzimuthCorrection(firing, dsr, azimuth_diff);
        azimuth_corrected = ((int)round(azimuth_corrected_f)) % 36000;

        FullPointType point;

        const double point_stamp_us =
            pkt_time_us + block * kVLP16BlockTduration +
            firing * kVLP16FiringToffset + dsr * kVLP16DsrToffset;

        point.time = static_cast<float>(point_stamp_us - msg_time_us);

        int ring = rings_map_[dsr];
        point.ring = ring;
        // point.angle = azimuth_corrected * 1.0 / 100;
        // point.angle = point.angle >= 0.0 ? point.angle : point.angle + 360.0;
        // point.angle = point.angle <= 360.0 ? point.angle : point.angle -
        // 360.0;
...
void PacketsParser::ArrangePointcloud(
    const std::vector<FullCloudPtr> &rings_pointcloud,
...
inline bool PacketsParser::isPointValid(const FullPointType &point) {
  if (point.x == HIT_NAN || point.x == HIT_FREE || point.y == HIT_NAN ||
      point.y == HIT_FREE || point.z == HIT_NAN || point.z == HIT_FREE)
    return false;
...
void PacketsParser::ComputeCoords(const float distance,
                                  const int vert_line_index,
