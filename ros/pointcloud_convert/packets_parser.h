...
public:
  ...
  void PaddingPointCloud(const PacketsMsgPtr &scan_msg,
                         FullCloudPtr &out_pc_msg_ptr);

private:
  /// In the output point cloud `rings_pointcloud`, the `curvature` field of
  /// each point stores the time difference between that point and `msg_stamp`
  ...
  void ComputeCoords(const float distance, const int vert_line_index,
                     const uint16_t &rotation, FullPointType &point);
