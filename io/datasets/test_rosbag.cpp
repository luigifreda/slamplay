...
void replayImageAndDepthSynchronized(slamplay::VioDatasetPtr &dataset) {
  ...
  constexpr int64_t sync_threshold_ns = 5 * 1e6; // milliseconds

  const std::vector<int64_t> &image_timestamps =
      dataset->get_image_timestamps();
  const std::vector<int64_t> &depth_timestamps =
      dataset->get_depth_timestamps();
  ...
  while (image_it != image_timestamps.end() &&
         depth_it != depth_timestamps.end()) {
...
int main(int argc, char **argv) {
  ...
  std::string dataset_path =
      "/home/luigi/Work/datasets/rgbd_datasets/tum/rgbd_dataset_freiburg1_xyz/"
  ...
  std::cout << "Reading " << dataset_type << " dataset: " << dataset_path
            << std::endl;

  slamplay::DatasetIoInterfacePtr dataset_io =
      slamplay::DatasetIoFactory::getDatasetIo(dataset_type);
  ...
  std::cout << "Found:" << std::endl
            << "\t " << dataset->get_image_timestamps().size() << " RGB images"
            << std::endl
            << "\t " << dataset->get_depth_timestamps().size()
            << " depth images" << std::endl
            << "\t " << dataset->get_accel_data().size() << " accel data"
            << std::endl
            << "\t " << dataset->get_gyro_data().size() << " gyro data"
            << std::endl;
  ...
  if (!gt_trajectory.empty()) {
    ...
    viz.start();
    viz.addTrajectory(gt_trajectory, slamplay::Trajectory::Color(0, 255, 0),
                      "gt");
  }
