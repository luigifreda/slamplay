...
class CholmodSolver {
   public:
    CholmodSolver(int size) : _msA(size, size),  // init _msA
                              _msAa(size, size),
                              _msI(size, size),
                              _vdb(size) {
   ...
   public:  // setters
    ...
    void SetAugmentedA(double lambda) {
        ...
        for (int j = 0; j < _msAa.cols(); j++)
        {
            ...
            dTrace += elemjj;
            if (elemjj < DBL_EPSILON) {
                vecDegenerateElems.push_back(j);
            }  // NOTE: no fabs needed! elemjj shoudl be >=0 coming from a positive-definite matrix
            elemjj *= lambdap1;
        }
