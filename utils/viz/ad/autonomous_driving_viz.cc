...
AutonomousDrivingViz::AutonomousDrivingViz() {
  impl_ = std::make_shared<AutonomousDrivingVizImpl>();
}
AutonomousDrivingViz::~AutonomousDrivingViz() {
...
bool AutonomousDrivingViz::Init() {
...
void AutonomousDrivingViz::Quit() {
...
void AutonomousDrivingViz::UpdatePointCloudGlobal(
...
void AutonomousDrivingViz::UpdateNavState(const NavStated &state) {
...
void AutonomousDrivingViz::UpdateScan(CloudPtr cloud, const SE3 &pose) {
...
void AutonomousDrivingViz::SetCurrentScanSize(int current_scan_size) {
  impl_->max_size_of_current_scan_ = current_scan_size;
}

void AutonomousDrivingViz::UpdateGps(const GNSS &gps) {
  std::lock_guard<std::mutex> lock(impl_->mtx_gps_pose_);
  impl_->gps_pose_ = gps.utm_pose_;
  impl_->gps_need_update_.store(true);
}

void AutonomousDrivingViz::SetTImuLidar(const SE3 &T_imu_lidar) {
  impl_->T_imu_lidar_ = T_imu_lidar;
}

bool AutonomousDrivingViz::ShouldQuit() { return pangolin::ShouldQuit(); }
