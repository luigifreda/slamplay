...
std::string print_info(const std::vector<T> &edges, double th) {
  ...
  boost::format fmt("count: %d, mean: %f, median: %f, 0.1 percentile: %f, "
                    "0.9 percentile: %f, 0.95 percentile: %f, max: %f, "
                    "threshold: %f\n");
  if (!chi2.empty()) {
    std::string str =
...
bool Optimization::Init(int stage) {
  ...
  std::vector<double> rtk_ext_t =
      yaml["rtk_ext"]["t"].as<std::vector<double>>();
...
void Optimization::Run() {
  ...
  BuildProblem(); // Build the problem

  SaveG2O(resultsLioMappingPath + "/before.g2o");
  LOG(INFO) << "RTK error: " << print_info(gnss_edge_, rtk_outlier_th_);
  LOG(INFO) << "RTK translation error: "
            << print_info(gnss_trans_edge_, rtk_outlier_th_);
  LOG(INFO) << "lidar error: " << print_info(lidar_edge_, 0);

  Solve();          // Solve once with robust kernel
  RemoveOutliers(); // Remove outliers
  Solve();          // Solve again

  SaveG2O(resultsLioMappingPath + "/after.g2o");
...
void Optimization::AddRTKEdges() {
  ...
  info_pos(2, 2) = 1.0 / (rtk_height_noise_ratio_ * rtk_pos_noise_ *
                          rtk_height_noise_ratio_ * rtk_pos_noise_);
  ...
  for (auto &kfp : keyframes_) {
    ...
    if (kf->rtk_heading_valid_) {
      ...
      auto edge = new EdgeGNSSTransOnly(vertices_.at(kf->id_),
                                        kf->rtk_pose_.translation(), TBG_);
  ...
  LOG(INFO) << "gnss edges: " << gnss_edge_.size() << ", "
            << gnss_trans_edge_.size();
}

void Optimization::AddLidarEdges() {
  const double lidar_pos_noise = 0.01,
               lidar_ang_noise =
                   0.1 * math::kDEG2RAD; // lidar observation noise
  Mat3d info_pos =
      Mat3d::Identity() * 1.0 / (lidar_pos_noise * lidar_pos_noise);
  Mat3d info_ang =
      Mat3d::Identity() * 1.0 / (lidar_ang_noise * lidar_ang_noise);
  Mat6d info_all = Mat6d::Identity();
  ...
  for (auto iter = keyframes_.begin(); iter != keyframes_.end(); ++iter) {
    ...
    for (int i = 0; i < lidar_continuous_num_; ++i) {
      ...
      // Add relative motion between iter and iter_next
      auto edge = new EdgeRelativeMotion(
          vertices_.at(iter->second->id_), vertices_.at(iter_next->second->id_),
...
void Optimization::AddLoopEdges() {
  ...
  const double loop_pos_noise = 0.1,
               loop_ang_noise = 0.5 * math::kDEG2RAD; // loop observation noise
...
void Optimization::Solve() {
  ...
  LOG(INFO) << "RTK translation error: "
            << print_info(gnss_trans_edge_, rtk_outlier_th_);
  LOG(INFO) << "lidar error: " << print_info(lidar_edge_, 0);
  LOG(INFO) << "loop error: " << print_info(loop_edge_, 0);
}

void Optimization::RemoveOutliers() {
  // Primarily remove GNSS outliers
  ...
  std::for_each(gnss_trans_edge_.begin(), gnss_trans_edge_.end(),
                remove_outlier);
  LOG(INFO) << "gnss outlier: " << cnt_outlier_removed << "/"
            << gnss_edge_.size() + gnss_trans_edge_.size();

  cnt_outlier_removed = 0;
  std::for_each(loop_edge_.begin(), loop_edge_.end(), remove_outlier);
  LOG(INFO) << "loop outlier: " << cnt_outlier_removed << "/"
            << loop_edge_.size();
}
...
void Optimization::SaveResults() {
  ...
  // Compare optimized pose vs RTK pose
...
void Optimization::LoadLoopCandidates() {
  std::ifstream fin(resultsLioMappingPath + "/loops.txt");
  if (!fin) {
    LOG(WARNING) << "cannot load file: " + resultsLioMappingPath +
                        "/loops.txt";
    return;
  }
