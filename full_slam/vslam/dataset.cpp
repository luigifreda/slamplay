...
bool DatasetEuroc::Init() {
  ...
  std::string timestamps_file_path =
      Config::Get<std::string>("timestamps_path");
  ...
  if (K_l.empty() || K_r.empty() || P_l.empty() || P_r.empty() || R_l.empty() ||
      ...
      cols_l == 0 || cols_r == 0) {
    LOG(FATAL) << "Calibration parameters to rectify stereo are missing!"
               << endl;
    return 1;
  }
  ...
  Camera::Ptr camera_right(
      new Camera(fx, fy, cx, cy, baseline, SE3(SO3(), tr)));
#if 0     
...
Frame::Ptr DatasetEuroc::NextFrame() {
  ...
  // read images
  image_left =
      cv::imread(vstrImageLeft[current_image_index_], cv::IMREAD_UNCHANGED);
  image_right =
      cv::imread(vstrImageRight[current_image_index_], cv::IMREAD_UNCHANGED);
...
bool DatasetEuroc::LoadImages(const string &strPathLeft,
...
bool DatasetEuroc::LoadImus(const string &strImuPath, VecIMU &vImus) {
  ...
  while (!fImus.eof()) {
    ...
    if (!s.empty()) {
      ...
      data[0] *= 1e-9;
      IMUData imudata(data[1], data[2], data[3], data[4], data[5], data[6],
                      data[0]);
...
bool DatasetEuroc::LoadGroundTruthTraj(const string &trajPath,
  ...
  while (!fTraj.eof()) {
    ...
    if (!s.empty()) {
      ...
      // x,y,z,qw,qx,qy,qz
      Sophus::SE3d pose(
          Sophus::SO3d(Eigen::Quaterniond(data[3], data[4], data[5], data[6])),
