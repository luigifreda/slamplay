...
inline void export_engine_sam_image_encoder(
    const std::string &f = "vit_l_embedding.onnx",
    const std::string &output = "vit_l_embedding.engine") {
  std::cout << "exporting " << f << " to " << output << std::endl;

  // create an instance of the builder
  std::unique_ptr<nvinfer1::IBuilder> builder(
      nvinfer1::createInferBuilder(slamplay::NvLogger::instance));
  // create a network definition
  // The kEXPLICIT_BATCH flag is required in order to import models using the
  // ONNX parser.
  uint32_t flag = 1U << static_cast<uint32_t>(
                      nvinfer1::NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);

  // auto network =
  // std::make_unique<nvinfer1::INetworkDefinition>(builder->createNetworkV2(flag));
  std::unique_ptr<nvinfer1::INetworkDefinition> network(
      builder->createNetworkV2(flag));

  // Importing a Model Using the ONNX Parser
  // auto parser =
  // std::make_unique<nvonnxparser::IParser>(nvonnxparser::createParser(*network,
  // slamplay::NvLogger::instance));
  std::unique_ptr<nvonnxparser::IParser> parser(
      nvonnxparser::createParser(*network, slamplay::NvLogger::instance));

  // read the model file and process any errors
  parser->parseFromFile(
      f.c_str(), static_cast<int32_t>(nvinfer1::ILogger::Severity::kWARNING));
  ...
  // Prefer FP32 for SAM to avoid FP16 layernorm instability on newer TensorRT
  // stacks.
  std::cout
      << "building SAM image encoder in FP32 for better numerical stability"
      << std::endl;

  // create an engine
  // auto serializedModel =
  // std::make_unique<nvinfer1::IHostMemory>(builder->buildSerializedNetwork(*network,
  // *config));
  std::unique_ptr<nvinfer1::IHostMemory> serializedModel(
      builder->buildSerializedNetwork(*network, *config));
  std::cout << "serializedModel->size(): " << serializedModel->size()
            << std::endl;
...
inline void export_engine_sam_sample_encoder_and_mask_decoder(
    const std::string &f = "sam_onnx_example.onnx",
    const std::string &output = "sam_onnx_example.engine") {
  std::cout << "exporting " << f << " to " << output << std::endl;
  ...
  //  Prefer FP32 for SAM to avoid FP16 layernorm instability on newer TensorRT
  //  stacks.
  std::cout << "building SAM prompt encoder/mask decoder in FP32 for better "
               "numerical stability"
            << std::endl;

  nvinfer1::IOptimizationProfile *profile =
      builder->createOptimizationProfile();
  // profile->setDimensions("image_embeddings",
  // nvinfer1::OptProfileSelector::kMIN, {1, 256, 64, 64 });
  // profile->setDimensions("image_embeddings",
  // nvinfer1::OptProfileSelector::kOPT, {1, 256, 64, 64 });
  // profile->setDimensions("image_embeddings",
  // nvinfer1::OptProfileSelector::kMAX, {1, 256, 64, 64 });

  profile->setDimensions("point_coords", nvinfer1::OptProfileSelector::kMIN,
                         {3, 1, 2, 2});
  profile->setDimensions("point_coords", nvinfer1::OptProfileSelector::kOPT,
                         {3, 1, 5, 2});
  profile->setDimensions("point_coords", nvinfer1::OptProfileSelector::kMAX,
                         {3, 1, 10, 2});

  profile->setDimensions("point_labels", nvinfer1::OptProfileSelector::kMIN,
                         {2, 1, 2});
  profile->setDimensions("point_labels", nvinfer1::OptProfileSelector::kOPT,
                         {2, 1, 5});
  profile->setDimensions("point_labels", nvinfer1::OptProfileSelector::kMAX,
                         {2, 1, 10});

  // profile->setDimensions("mask_input", nvinfer1::OptProfileSelector::kMIN, {
  // 1, 1, 256, 256}); profile->setDimensions("mask_input",
  // nvinfer1::OptProfileSelector::kOPT, { 1, 1, 256, 256 });
  // profile->setDimensions("mask_input", nvinfer1::OptProfileSelector::kMAX, {
  // 1, 1, 256, 256 });

  // profile->setDimensions("has_mask_input",
  // nvinfer1::OptProfileSelector::kMIN, { 1,});
  // profile->setDimensions("has_mask_input",
  // nvinfer1::OptProfileSelector::kOPT, { 1,});
  // profile->setDimensions("has_mask_input",
  // nvinfer1::OptProfileSelector::kMAX, { 1,});
