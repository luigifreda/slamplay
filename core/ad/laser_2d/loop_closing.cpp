...
#include "macros.h"
std::string loopClosingResultsDir =
    STR(RESULTS_DIR) +
...
bool LoopClosing::DetectLoopCandidates() {
  // Require a certain gap between current frame and historical submaps
  ...
  for (auto &sp : submaps_) {
    if ((last_submap_id_ - sp.first) <= submap_gap_) {
      // Skip the most recent submaps
      continue;
    }

    // Skip if a valid association already exists with this historical submap
    ...
    if (dis < candidate_distance_th_) {
      /// If the frame is within the distance threshold from the submap center,
      /// check it
...
void LoopClosing::MatchInHistorySubmaps() {
  // Save scans, poses and submaps to offline files for debugging MR match
  // before going live
  current_frame_->Dump(loopClosingResultsDir + "/frame_" +
...
void LoopClosing::Optimize() {
  ...
  for (auto &sp : submaps_) {
    ...
    // Update the world poses of frames belonging to this submap
