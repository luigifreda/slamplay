...
void Submap::SetOccuFromOtherSubmap(std::shared_ptr<Submap> other) {
  ...
  // Take the most recent 10 frames
  for (size_t i = frames_in_other.size() - 10; i < frames_in_other.size();
       ++i) {
    if (i > 0) {
...
void Submap::AddScanInOccupancyMap(std::shared_ptr<Frame> frame) {
  occu_map_.AddLidarFrame(
      frame, OccupancyMap::GridMethod::MODEL_POINTS); // update cells in the
                                                      // occupancy grid map
  field_.SetFieldImageFromOccuMap(
      occu_map_.GetOccupancyGrid()); // update the field function image
}
