...
void MRLikelihoodField::BuildModel() {
  ...
  /// Generate template pyramid images
  field_ = {
...
bool MRLikelihoodField::AlignG2O(SE2 &init_pose) {
  ...
  /// If matching succeeds, print some info
  for (int l = 0; l < levels_; ++l) {
...
bool MRLikelihoodField::AlignInLevel(int level, SE2 &init_pose) {
  ...
  const double range_th = 15.0; // ignore distant scans, unreliable
  ...
  /// Count how many edges are inliers
  int num_inliers =
  ...
  /// Require the inlier ratio to exceed a threshold
  /// This is subtle because the lidar is not 360-degree, so most of the time
  /// only partial matching is possible
