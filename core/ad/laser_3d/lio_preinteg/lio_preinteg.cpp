...
void LioPreinteg::Align() {
  ...
  if (ui_) {
    ui_->UpdateScan(current_scan_,
                    current_nav_state_.GetSE3()); // pass pose in LiDAR frame
...
void LioPreinteg::TryInitIMU() {
  ...
  if (imu_init_.InitSuccess()) {
    // read initial biases; noise from static IMU init
...
void LioPreinteg::Undistort() {
  ...
  auto imu_state = imu_states_.back(); // state at scan end time
  ...
  std::for_each(std::execution::par_unseq, cloud->points.begin(),
                cloud->points.end(), [&](auto &pt) {
                  ...
                  // pt.time is ms offset from scan start; interpolate pose at hit time
...
void LioPreinteg::LivoxPCLCallBack(
...
void LioPreinteg::Optimize() {
  // g2o: preintegration between last and current state, NDT pose factor at current
  LOG(INFO) << " === optimizing frame " << frame_num_ << " === "
            << ", dt: " << preinteg_->dt_;

  /// NOTE: information matrices are sensitive; wrong scales often stall optimization
  ...
  // vertices at previous time: pose, v, bg, ba
  ...
  // vertices at current time: pose, v, bg, ba
  auto v1_pose = new VertexPose();
  v1_pose->setId(4);
  v1_pose->setEstimate(ndt_pose_); // NDT pose as initial value
  // v1_pose->setEstimate(current_nav_state_.GetSE3());  // predicted pose as initial
  ...
  // prior on previous frame pose, vel, bg, ba
  ...
  // marginalize to get prior on current state (build Hessian, then marginalize)
  // variable order: v0_pose, v0_vel, v0_bg, v0_ba, v1_pose, v1_vel, v1_bg, v1_ba
...
void LioPreinteg::NormalizeVelocity() {
  /// clamp velocity (typically forward speed along body -y)
