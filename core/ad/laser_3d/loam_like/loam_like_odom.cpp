...
void LoamLikeOdom::ProcessPointCloud(FullCloudPtr cloud) {
  ...
  pcl::transformPointCloud(*ConvertToCloud<FullPointType>(cloud), *scan_world,
                           pose.matrix());
...
bool LoamLikeOdom::IsKeyframe(const SE3 &current_pose) {
  ...
  // Mark as keyframe if relative motion from the last frame exceeds a distance
  // or angle threshold
...
SE3 LoamLikeOdom::AlignWithLocalMap(CloudPtr edge, CloudPtr surf) {
  // This ICP part is implemented manually
  SE3 pose;
  ...
  for (int iter = 0; iter < options_.max_iteration_; ++iter) {
    ...
    std::vector<Eigen::Matrix<double, 3, 6>> jacob_edge(
        edge_size); // point-to-line residual is 3D
    std::vector<Vec3d> errors_edge(edge_size);

    std::vector<int> index_surf(surf_size);
    std::iota(index_surf.begin(), index_surf.end(),
              0); // fill with sequential indices
    std::vector<int> index_edge(edge_size);
    std::iota(index_edge.begin(), index_edge.end(),
              0); // fill with sequential indices

    // Gauss-Newton iteration
    // Nearest-neighbor search, edge points
    if (options_.use_edge_points_) {
      std::for_each(
          std::execution::par_unseq, index_edge.begin(), index_edge.end(),
          [&](int idx) {
    ...
    /// Nearest-neighbor search, planar points
    if (options_.use_surf_points_) {
      std::for_each(
          std::execution::par_unseq, index_surf.begin(), index_surf.end(),
          [&](int idx) {
    ...
    // Could use parallel reduce in principle, but accumulate is simpler here
    double total_res = 0;
    int effective_num = 0;
