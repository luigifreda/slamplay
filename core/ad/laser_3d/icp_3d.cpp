...
bool Icp3d::AlignP2Plane(SE3 &init_pose) {
  ...
  assert(target_ != nullptr && source_ != nullptr);
  // overall process is the same as P2P, note the differences
  ...
  for (int iter = 0; iter < options_.max_iteration_; ++iter) {
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(),
                  [&](int idx) {
                    ...
                    Vec3d qs = pose * q; // q after transformation
                    std::vector<int> nn;
                    kdtree_->GetClosestPoint(ToPointType(qs), nn,
                                             5); // get 5 nearest neighbors
                    if (nn.size() > 3) {
                      // convert to eigen
...
bool Icp3d::AlignP2Line(SE3 &init_pose) {
  ...
  assert(target_ != nullptr && source_ != nullptr);
  // point-to-line is basically identical to point-to-plane
