...
void GinsPreInteg::SetOptions(sad::GinsPreInteg::Options options) {
  ...
  options_.gnss_info_.diagonal() << 1.0 / ga2, 1.0 / ga2, 1.0 / ga2, 1.0 / gp2,
      1.0 / gp2, 1.0 / gh2;
...
void GinsPreInteg::AddGnss(const GNSS &gnss) {
  ...
  if (!first_gnss_received_) {
    if (!gnss.heading_valid_) {
      // The first GNSS measurement must contain heading
      return;
    }

    // For the first GNSS signal, initialize the pose from this measurement
  ...
  // Integrate up to the GNSS timestamp
...
void GinsPreInteg::Optimize() {
  if (pre_integ_->dt_ < 1e-3) {
    // No integration result available
    return;
  }
  ...
  if (last_odom_set_) {
    ...
    // Reset the odometry-arrival flag and wait for the latest odometry data
  ...
  if (options_.verbose_) {
    // Collect results and summarize each error term
    LOG(INFO) << "chi2/error: ";
