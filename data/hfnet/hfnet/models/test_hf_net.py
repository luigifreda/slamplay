...
def local_head(prefix, features, config):
    bn_params = {
    ...
    return {prefix + 'local_descriptor_map': desc, prefix + 'logits': logits,
            prefix + 'prob_full': prob_full, prefix + 'scores_dense': prob}
...
class HfNet(BaseModel):
    ...
    def _model(self, inputs, mode, **config):
        ...
        n_level = 2
        scale_factor = tf.constant(1.2, name='scale_factor')
        scale_factor_inv = 1 / scale_factor

        local_feat_level = [None for i in range(n_level)]
        local_feat_level[0] = encoder[config['local_endpoint']]
        for level in range(1, n_level):
            with tf.name_scope('resize_output'):
                new_shape = tf.cast(tf.math.round(tf.cast(tf.shape(local_feat_level[level-1])[1:3], tf.float32) * scale_factor_inv), tf.int32)
                local_feat_level[level] = tf.image.resize_bilinear(local_feat_level[0], new_shape)
        ...
        with tf.variable_scope('local_head', reuse=tf.AUTO_REUSE), \
            ...
            ret_local = {**ret_local, **local_head('level{}_'.format(1), local_feat_level[1], config['local'])}
        
        ret = {**ret_global, **ret_local}
        # if mode == Mode.PRED:
        #     ret['scores_dense'] = simple_nms(ret['scores_dense'], config['local']['nms_radius'], 1)
